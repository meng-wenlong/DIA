...
def eval_result_with_evaluator(
    ...
    for result in results:
        instructs.append(result["instruct"])
...
def save_results(eval_results: list[bool], result_path: str, preset_name: str) -> None:
    try:
        with open(result_path, "r", encoding="utf-8") as file:
            results = json.load(file)
        
        if len(eval_results) != len(results):
            raise ValueError("Length of 'eval_results' does not match the number of entries in the results file.")

        for i, result in enumerate(results):
            result["eval_" + preset_name] = eval_results[i]
        ...
        print(f"Error: The file {result_path} is not a valid JSON file.")
...
def main() -> None:
    ...
    if evaluator:
        for rewritten_path in rewritten_paths:
            ...
            # write eval_results to rewritten_path
            save_results(
                eval_results, 
                rewritten_path, 
                args.preset_path.split(os.sep)[-1]
            )
