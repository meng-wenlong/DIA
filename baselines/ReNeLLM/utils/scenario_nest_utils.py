...
def extract_instruct(nested_prompt):
    for scenario in SCENARIOS:
        # Escape special regex characters in the scenario template
        escaped_scenario = re.escape(scenario)
        # Replace the escaped placeholder '<>' with a named regex group
        escaped_placeholder = re.escape('<>')
        pattern = escaped_scenario.replace(escaped_placeholder, '(?P<instruct>.*?)')
        # Compile the regex pattern with DOTALL flag to match across newlines
        regex = re.compile(pattern, re.DOTALL)
        # Try to match the nested_prompt with the pattern
        match = regex.match(nested_prompt)
        if match:
            # Extract the 'instruct' group from the match
            instruct = match.group('instruct')
            return instruct
